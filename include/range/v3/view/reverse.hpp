    ...
    inline namespace v3
    {
                ...
                iterator_t<Rng>, reverse_view<Rng>, !BoundedRange<Rng>()>
        {
            ...
            constexpr range_size_type_t<Rng> size() const
                RANGES_NOEXCEPT(noexcept(ranges::size(std::declval<Rng const &>())))
            {
            ...
            RANGES_CXX14_CONSTEXPR range_size_type_t<Rng> size()
                RANGES_NOEXCEPT(noexcept(std::declval<reverse_view &>().size_(SizedRange<Rng>())))
            {
        ...
        private:
            ...
            RANGES_CXX14_CONSTEXPR iterator_t<Rng> get_end()
                RANGES_NOEXCEPT(noexcept(std::declval<reverse_view &>().get_end_(BoundedRange<Rng>())))
            {
            ...
            public:
                ...
                void advance(iterator_t<Rng> &it, range_difference_type_t<Rng> n) const
                    RANGES_NOEXCEPT(noexcept(ranges::advance(it, -n)))
                {
                ...
                distance_to(iterator_t<Rng> const &here, iterator_t<Rng> const &there,
                    ...
                    RANGES_NOEXCEPT(noexcept(here - there))
                {
            ...
            RANGES_CXX14_CONSTEXPR adaptor begin_adaptor() const
                RANGES_NOEXCEPT((std::is_nothrow_constructible<adaptor, reverse_view const &>::value))
            {
                return {*this};
            }
            CONCEPT_REQUIRES(const_iterable)
            ...
            RANGES_CXX14_CONSTEXPR adaptor begin_adaptor()
                RANGES_NOEXCEPT((std::is_nothrow_constructible<adaptor, reverse_view &>::value))
            {
                return {*this};
            }
            CONCEPT_REQUIRES(!const_iterable)
