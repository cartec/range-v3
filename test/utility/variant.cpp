...
#define ASSERT_SAME_TYPE(...) STATIC_ASSERT(std::is_same<__VA_ARGS__>::value)
...
namespace get_index
{
    void test_const_lvalue_get()
    {
        ...
        }
        {
            using V = ranges::variant<int, const long>;
            constexpr V v(42l);
            ASSERT_SAME_TYPE(decltype(ranges::get<1>(v)), const long &);
            STATIC_ASSERT(ranges::get<1>(v) == 42);
        }
        {
            using V = ranges::variant<int, const long>;
            const V v(42l);
            ASSERT_NOT_NOEXCEPT(ranges::get<1>(v));
            ASSERT_SAME_TYPE(decltype(ranges::get<1>(v)), const long &);
    ...
    bool ex_test(std::integral_constant<std::size_t, I>, V &&v) {
        try {
            TEST_IGNORE_NODISCARD ranges::get<I>(std::forward<V>(v));
...
namespace visit
{
    ...
    void test_call_operator_forwarding()
    {
        ...
        { // test call operator forwarding - no variant
            ranges::visit(obj);
            CHECK(Fn::check_call<>(CT_NonConst | CT_LValue));
            ranges::visit(cobj);
            CHECK(Fn::check_call<>(CT_Const | CT_LValue));
            ranges::visit(std::move(obj));
            CHECK(Fn::check_call<>(CT_NonConst | CT_RValue));
            ranges::visit(std::move(cobj));
            CHECK(Fn::check_call<>(CT_Const | CT_RValue));
        }
    ...
    struct ReturnFirst
    {
        template <class T, class... Args> constexpr int operator()(T f, Args &&...) const
        {
